...
firstUpdate.querySelector(".edit-box").addEventListener("click", () => {
  ...
  firstUpdate
    .querySelector(".my-update-reactions")
    .classList.add("update-focus");
});
...
let makeComment = (id, score, image, username, createdAt, content) => {
  ...
  comment.innerHTML = `
    <div id = ${id} class =  ${
    username === datas.currentUser.username ? "my-first-box" : "first-box"
  }>
  ...
  if (comment.querySelector(".first-box").id == 1) {
    comment.querySelector(".reply-box").addEventListener("click", () => {
      ...
      document.querySelector(".my-reply").addEventListener("click", () => {
        document.querySelector(".my-first-update-massage").innerHTML = `${
          replyBox.querySelector(".my-reply-massage").value
        }`;
        document.querySelector(".reply-conteiner").style.display = "none";
        document.querySelector(".my-first-update-conteiner").style.display =
          "flex";
      });
    });
  }
...
let addFirstComment = makeComment(
  firstComment.id,
  firstComment.score,
  firstComment.user.image.png,
...
// create first comment
let addSecondComment = makeComment(
  secondComment.id,
  secondComment.score,
  secondComment.user.image.png,
  secondComment.user.username,
...
<textarea class = "my-update-massage" disabled>${
    replyBoxSecond.querySelector(".my-reply-massage").value
  }</textarea>
<button class = "update-update">UPDATE</button>
<div class = "my-update-reactions">
  <img src = "images/icon-plus.svg" class = "plus">
...
<span class = "update-buttons">
  ...
  updateReply.querySelector(".minus").addEventListener("click", () => {
    updateReply.querySelector(".reactions-count").innerHTML = reactCountSecond;
    if (reactCountSecond > 0) reactCountSecond--;
  });
...
replyCom.querySelector(".minus").addEventListener("click", () => {
  replyCom.querySelector(".reactions-count").innerHTML = reactionCountRamse;
  if (reactionCountRamse > 0) reactionCountRamse--;
});
